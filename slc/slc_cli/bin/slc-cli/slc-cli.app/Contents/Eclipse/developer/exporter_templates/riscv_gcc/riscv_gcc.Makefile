...
ifeq ($(OS),Windows_NT)
  RISCV_GCC_DIR ?= $(RISCV_GCC_DIR_WIN)
	POST_BUILD_EXE ?= $(POST_BUILD_EXE_WIN)
else
  UNAME_S := $(shell uname -s)
  ifeq ($(UNAME_S),Darwin)
    RISCV_GCC_DIR ?= $(RISCV_GCC_DIR_OSX)
		POST_BUILD_EXE ?= $(POST_BUILD_EXE_OSX)
  else
    RISCV_GCC_DIR ?= $(RISCV_GCC_DIR_LINUX)
...
AR      = "$(RISCV_GCC_DIR)/bin/riscv32-unknown-elf-gcc-ar"
CC      = "$(RISCV_GCC_DIR)/bin/riscv32-unknown-elf-gcc"
CXX     = "$(RISCV_GCC_DIR)/bin/riscv32-unknown-elf-g++"
OBJCOPY = "$(RISCV_GCC_DIR)/bin/riscv32-unknown-elf-objcopy"
LD      = "$(RISCV_GCC_DIR)/bin/riscv32-unknown-elf-gcc"

####################################################################
# Include sub-makefiles                                            #
# Define a makefile here to add files/settings to the build.       #
####################################################################
{% for mak in INLCUDED_MAKEFILES %}-include {{mak}}{% endfor %}
