...
target_compile_definitions(openthread-efr32-rcp PUBLIC {{ openthread_device_type() }}
)

target_compile_definitions(silabs-efr32-sdk-rcp PRIVATE {{ openthread_device_type() }}
)
{%- endif %}
...
{%- if linker_flags -%}
target_link_options(openthread-efr32-rcp PRIVATE {{ print_linker_flags() }}
)
{%- endif %} {#- linker_flags #}
