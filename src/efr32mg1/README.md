...
This directory contains example platform drivers for the [Silicon Labs EFR32MG1P][efr32mg] based on [EFR32™ Mighty Gecko Wireless Starter Kit][slwstk6000b].
...
$ ./script/build efr32mg1 -DBOARD=brd4151a
...
-- Configuring done
-- Generating done
-- Build files have been written to: <path-to-ot-efr32>/build/efr32mg1
+ [[ -n ot-rcp ]]
+ ninja ot-rcp
[160/160] Linking CXX executable bin/ot-rcp
+ cd <path-to-ot-efr32>
```

After a successful build, the `elf` files are found in `<path-to-ot-efr32>/build/efr32mg1/bin`.

```bash
$ ls build/efr32mg1/bin
ot-rcp  ot-rcp.s37
```
...
```bash
$ cd <path-to-ot-efr32>
$ ./script/build efr32mg1 -DBOARD=brd4151a -DOT_COMMISSIONER=ON -DOT_JOINER=ON -DOT_DHCP6_CLIENT=ON -DOT_DHCP6_SERVER=ON
```
